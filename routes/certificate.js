...
router.get("/generate-all-qrcodes", async (req, res) => {
    try {
        const allCertificates = await Certificate.find({}).exec();
        const basePath = "./uploads/certificates/";

        for (const cert of allCertificates) {
            const certDir = path.join(basePath, cert.certificateNo);
            ...
            // Generate QR code
            const qrPath = path.join(certDir, "qrcode.png");
            await qrcode.toFile(qrPath, `${process.env.BASE_URL}/certificate/view/${cert.certificateNo}`);
            console.log(`✅ QR code generated for: ${cert.certificateNo}`);
        }

        res.send("✅ QR Codes generated for all certificates.");
    } catch (error) {
        console.error("❌ Error generating QR codes:", error);
        res.status(500).send("Something went wrong while generating QR codes.");
    }
});
...
router.post("/insert", async (req, res) => {
    ...
    try {
        ...
        await qrcode.toFile("./uploads/certificates/" + body['certificateNo'] + "/qrcode.png", `${process.env.BASE_URL}/certificate/view/` + body['certificateNo']);
...
router.post("/update/:certificateNo", async (req, res) => {
    try {
        ...
        await qrcode.toFile("./uploads/certificates/" + certificateNo + "/qrcode.png", `${process.env.BASE_URL}/certificate/view/` + certificateNo);
